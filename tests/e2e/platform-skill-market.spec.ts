...
test.describe('platform skill marketplace end to end', () => {
  ...
  test('uploads a skill package and shows it in my packages', async ({ page }) => {
    ...
    await expect(page.locator('.page-header h1')).toHaveText('上传技能');
    ...
    await expect(page.getByRole('heading', { name: '上传成功' })).toBeVisible();

    await page.getByRole('button', { name: '我的技能包' }).click();
  ...
  test('adds a git-backed source for publishing', async ({ page }) => {
    ...
    await expect(page.locator('.badge', { hasText: '发布目标' })).toBeVisible();
  });
