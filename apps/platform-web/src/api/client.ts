...
export interface AuthConfig { enabled: boolean; mode?: 'oauth' | 'local' | 'none'; scopes: string[]; apiAvailable: boolean; }
...
export async function getAuthConfig(): Promise<AuthConfig> { try { const raw = await request<Record<string, unknown>>('/api/auth/config'); return { enabled: raw.enabled === true, mode: raw.mode === 'oauth' ? 'oauth' : raw.mode === 'local' ? 'local' : raw.mode === 'none' ? 'none' : undefined, scopes: tags(raw.scopes), apiAvailable: true }; } catch { return { enabled: false, scopes: [], apiAvailable: false }; } }
