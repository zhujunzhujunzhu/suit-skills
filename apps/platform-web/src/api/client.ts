...
export async function listMySkills(owner = 'current-user'): Promise<SkillItem[]> { try { const payload = await request<unknown>(`/api/skills?owner=${encodeURIComponent(owner)}`); const items = isRecord(payload) && Array.isArray(payload.items) ? payload.items : []; return items.map(normalizeSkill); } catch { return readLocalSkills().filter((item) => item.owner === owner || item.source === 'default'); } }
