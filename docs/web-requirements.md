...
`Installed`、本地详情、导出、删除等以本地安装数据为准的页面或操作，不得依赖远程 source 刷新完成后才能工作。

要求：

- 远程 source 不可达、超时或刷新失败时，本地已安装数据仍必须可用。
- `sourceName` 等展示字段应优先来自本地缓存或本地安装记录，不能为了补齐展示信息而触发额外远程扫描。
- 本地视图不能被远程刷新失败污染，也不能因为远程刷新变慢而被阻塞。

### 3.5 共享读取去重与短时缓存

同一交互链路内重复使用的只读数据应复用，避免在列表页、详情页、切换页签时重复读取相同状态。

要求：

- 列表页、详情页、Installed 页之间共享的已安装状态应优先复用，而不是重复读取。
- 桌面 / Tauri 模式下应尽量减少重复 IPC 和 sidecar 启动；允许使用短生命周期缓存，但在安装、删除、链接、刷新等变更后必须失效。
- 新增接口设计时，应优先考虑组合读取或批量读取，避免前端通过多次小请求拼装核心页面。

### 3.6 只保留最新请求，过时请求必须可取消

搜索、source 切换、详情切换、Installed 切换等会被频繁打断的读取请求，必须支持取消，而不只是忽略旧响应。

要求：

- 前端应主动中止过期请求，减少浏览器、网络和后端无效工作。
- UI 状态必须以最后一次用户操作为准，不能被旧请求回写覆盖。
- 请求取消能力需要覆盖技能库、Installed 列表和详情读取等主要高频交互路径。

### 3.7 主路径优先与体积控制

首屏主路径只加载当前必需代码，非主路径视图应按需加载，避免将大部分视图与状态长期堆叠在单一入口 chunk 中。

要求：

- `download`、`skill-detail` 以及翻译内容较重的面板应优先考虑懒加载。
- Web 构建需要持续关注主 bundle 体积与 chunk 拆分，桌面模式需要持续关注 IPC 次数与 sidecar 启动次数。
- 重建产物时应清理旧哈希资源，避免 `dist/web/assets` 持续累积历史文件。
...
下载能力继续保留独立页面，但入口位置调整为顶部栏，不再放在左侧导航中。

顶部栏要求：

- 在语言切换、设置、刷新等全局操作附近提供 `下载` 入口。
- `下载` 入口右侧展示当前运行端的版本号。
- Web 端运行时展示当前 Web 版本。
- 桌面端运行时展示当前桌面应用版本。
...
- Web 端需要额外检测当前 Web 版本是否落后于 npm 已发布的最新版本。
- 桌面端需要检测当前桌面应用版本是否落后于桌面构建发布清单中的最新版本。
- 如果当前版本不是最新版本，顶部栏下载入口需要有明显但克制的提示标识，例如状态点、提示徽标或最新版本标签。
- 即使用户关闭一次更新横幅，顶部栏仍应继续保留版本状态提示，直到当前版本更新为止。

下载页面要求：

- 下载页面继续保留完整的桌面安装包列表与构建说明。
- 下载页面标题、副标题或版本摘要区需要与顶部栏版本状态保持一致，避免顶部栏和页面内文案相互矛盾。

### 15.4 Installed 搜索字段与过滤行为
